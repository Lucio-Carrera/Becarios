<?php
        ...
        if (isset($_GET['idalumno']) && isset($_GET['anoreg']) && isset($_GET['toficio']) )
    {
       ...
       $consulta="SELECT A.ID_Est, A.Ano_Reg, A.ApPaterno_Est, A.ApMaterno_Est, A.Nombre_Est, B.ID_Ger3, B.ID_Sub3, B.ID_Dir3, B.Extension, B.F_Inicio, B.F_Final, B.Hr_Inicio, B.Hr_Final, B.H_Especifico, B.Ubicacion FROM datospersonales A, datosarea B WHERE B.ID_Estudiante3= $idalumno and A.ID_Est=$idalumno and A.Ano_Reg = $anoreg";
...
Coordinación de Profesionistas en Entrenamiento', 0, 'J', false);
        ...
        $pdf->MultiCell(85,7,$fecha, 0, 'R', false); //Imprime la fecha
        $pdf->Ln(25);
        ...
        $pdf->SetFont('narrow','',9);
        $pdf->SetXY(34, 88);
        $pdf->MultiCell(80,4,'SOLICITUD PASE DE ACCESO PROFESIONISTA EN ENTRENAMIENTO C. '.utf8_decode($row['Nombre_Est']).' '.utf8_decode($row['ApPaterno_Est']).' '.utf8_decode($row['ApMaterno_Est']), 0, 'L', false);
        ...
        $pdf->SetXY(55, 140);
        $pdf->MultiCell(145,5,''.($row['ID_Ger3']).' de la '.($row['ID_Sub3']).' de la '.($row['ID_Dir3']), 0, 'J', false); 
        $pdf->SetXY(15, 150);
       ...
       /*$pdf->MultiCell(40,5,'Horario Específico:', 1, 'J', false);
        $pdf->SetXY(55, 155);
        $pdf->MultiCell(146,5,'(horario especifico)' , 1, 'J', false);*/

        $pdf->SetXY(15,165);
        $pdf->MultiCell(40,10,'', 1, 'J', false);
        $pdf->SetXY(15, 165);
        $pdf->MultiCell(40,5,'Horario Específico:', 0, 'J', false);
        $pdf->SetXY(55,165);
        $pdf->MultiCell(146,10,'', 1, 'J', false);
        $pdf->SetXY(55, 165);
        $pdf->MultiCell(165,5,''.utf8_decode($row['H_Especifico']), 0, 'J', false);

        /*
        $pdf->SetXY(60, 165);
        $pdf->MultiCell(146,19,'', 1, 'J', false);
        $pdf->SetXY(60, 165);
        $pdf->MultiCell(165,4,'Horario Específico' , 0, 'J', false);*/
        ...
        $pdf->SetXY(55, 175);
        $pdf->MultiCell(146,4,''.utf8_decode($row['Ubicacion']), 0, 'J', false); 
...
Coordinación de Profesionistas en Entrenamiento', 0, 'J', false);
        ...
        $pdf->SetFont('narrow','',8);
        $pdf->SetXY(144, 32);
        $pdf->MultiCell(64,4,'DCAS-SRH-GOCH-SRDHA-CPE-'.$oficio, 0, 'J', false);
...
*/
        ...
        $consulta3="SELECT * FROM oficioguardado WHERE idalumno='$idalumno' AND anoregistro='$anoreg' AND toficio='PASE DE ACCESO'";
        ...
        {   
            ...
            if ($cont>0) 
            {
                ...
                $query ="UPDATE OficioGuardado SET toficio='PASE DE ACCESO',  idalumno='$idalumno', anoregistro='$anoreg', dia='$dia', mes='$mes', ano='$anio', ofisal='$oficio', cargo='$cargo', firmaofi='$firma', felaboracion='$felaboracion', PE='$PE',  WHERE idalumno='$idalumno' AND anoregistro='$anoreg' AND toficio='PASE DE ACCESO'";
            ...
            {
                $query = "INSERT INTO OficioGuardado (toficio, idalumno, anoregistro, dia, mes, ano, ofisal, cargo, firmaofi,felaboracion, PE) values ('PASE DE ACCESO', '$idalumno', '$anoreg', 
                '$dia' , '$mes' ,'$anio','$oficio',  '$cargo', '$firma','$felaboracion', '$PE')";
