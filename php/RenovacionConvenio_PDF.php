...
GERENCIA DE RECLUTAMIENTO Y SELECCIÓN', 0, 'C', false);
      ...
      $pdf->MultiCell(171,4, "Petróleos exicanos es una empresa productiva del Estado con régimen especial, de propiedad exclusiva del Gobierno Federal, con personalidad jurídica y patrimonio propios, que goza de autonomía técnica, operativa y de gestión, cuyo objeto consiste en llevar a cabo la exploración y extracción del petróleo y de los carburos de hidrógeno sólidos, líquidos o gaseosos, así como su recolección, venta y comercialización, y tiene como fin el desarrollo de actividades empresariales, económicas, industriales y comerciales en términos de dicho objeto, generando valor económico y rentabilidad, para lo cual cuenta con la capacidad para celebrar toda clase de actos jurídicos, entre otros, convenios, contratos, alianzas y asociaciones, con personas físicas o morales de los sectores público, privado o social, nacional o internacional, en términos de la Ley de Petróleos Mexicanos, publicada en Diario Oficial de la Federación el 11 de agosto de 2014.", 0, 'J', false);
        ...
        $pdf->SetFont('narrow','',10); //Arial, negrita, 12 puntos
        $pdf->SetXY(28, 162);
        $pdf->MultiCell(171,4,'Tiene interés de ser entrenado por PEMEX para continuar con su desarrollo profesional, a través de la instrucción teórica y práctica requerida para el Proyecto '.utf8_decode($row['Proyecto']).". y a la vez, obtener el beneficio de una ayuda económica. ", 0, 'J', false);
...
'.utf8_decode($row['CargoGerente']), 0, 'C', false);
        ...
        $anoreg=$_POST['anoreg']; 
                    ...
                    $consulta="SELECT A.ID_Est, A.Ano_Reg, A.ApPaterno_Est, A.ApMaterno_Est, A.Nombre_Est, A.Egresado, A.Carrera, A.ID_Institucion2, A.Plantel, A.Creditos, A.Doctor, A.Ceduladr, A.Domicilio, A.Colonia, A.Delegacion, A.CP, A.Estado, B.Proyecto, B.ID_Ger3, B.ID_Sub3, B.ID_Dir3, B.Nombre_Asesor, B.Cvedepto, B.F_InicioRen, B.F_FinalRen, B.F_Convenio, B.Hr_Inicio, B.Hr_Final, B.Ubicacion, B.Nombre_Gerente, B.CargoGerente FROM datospersonales A, datosarea B WHERE A.ID_Est=$idalumno and B.ID_Estudiante3=$idalumno and A.Ano_Reg=$anoreg";
                    $resid=$conexion->query($consulta);
                    $row=mysqli_fetch_array($resid);
       $date1=explode("-", $row['F_InicioRen']);    
        $date2=explode("-", $row['F_FinalRen']);        
...
'.utf8_decode($row['CargoGerente']), 0, 'C', false);
        ...
        $consulta3="SELECT * FROM OficioGuardado WHERE idalumno='$idalumno' AND anoregistro='$anoreg' AND toficio=' RENOVACION CONVENIO'";
        ...
        {   
            ...
            if ($cont>0) 
            {
                ...
                $query ="UPDATE oficioguardado SET toficio=' RENOVACION CONVENIO', idalumno='$idalumno', anoregistro='$anoreg' WHERE idalumno='$idalumno' AND anoregistro='$anoreg' AND toficio=' RENOVACION CONVENIO'";
            ...
            {
                $query = "INSERT INTO OficioGuardado (toficio, idalumno, anoregistro) values (' RENOVACION CONVENIO', '$idalumno', '$anoreg')";
                $resultado=mysqli_query($conexion, $query);
                if (!$resultado)
                {
